...
var FILE_FORMATS=['RVT','IFC','DWG','DXF','PDF','XLSX','DOCX','PNG','JPG','NWD','NWC','SKP','OBJ','CSV'];
...
function renderDeliverables(){
  ...
  var schemaFilters=APP.schemas
    ...
    .sort(function(a,b){return a.code_order-b.code_order;})
    .map(function(s){
      var cur=APP.fieldFilters[s.key]||'';
      var opts='<option value="">'+s.name+'</option>';
      if(s.allowed_values){
        opts+=s.allowed_values.map(function(v){
          return '<option value="'+v.value+'"'+(cur===v.value?' selected':'')+'>'+v.value+' - '+v.label+'</option>';
        }).join('');
      }
      return '<select class="input" style="width:120px;font-size:11px" onchange="setFieldFilter(\''+s.key+'\',this.value)">'+opts+'</select>';
    }).join('');
  ...
  document.getElementById('content').innerHTML=
    ...
    '<select class="input" style="width:120px;font-size:11px" onchange="APP.statusFilter=this.value;loadDeliverables()">'+
    '<option value="">Todos estados</option>'+
    Object.entries(STATUS_CFG).map(function(e){return '<option value="'+e[0]+'">'+e[1].label+'</option>';}).join('')+
    '</select>'+
    schemaFilters+
    '<button class="btn btn-sm" onclick="clearFilters()" title="Limpiar filtros">x Limpiar</button>'+
    '<button class="btn btn-sm" onclick="loadDeliverables()">Actualizar</button></div>'+
    '<div id="del-table">'+loading()+'</div>';
  loadDeliverables();
}

function setFieldFilter(key,val){
  APP.fieldFilters[key]=val;
  loadDeliverables();
}

function clearFilters(){
  APP.fieldFilters={};APP.search='';APP.statusFilter='';
  renderDeliverables();
}
...
function loadDeliverables(){
  ...
  Promise.all([
    ...
    var html='<div style="border-radius:var(--rl);border:1px solid var(--border);background:var(--surface);overflow:hidden">'+
      ...
      items.map(function(d){
        var statusCell=canStatus
          ?'<select class="input" style="width:105px;font-size:10px;padding:2px 18px 2px 5px" onchange="changeStatus(\''+d.id+'\',this.value)">'+
        ...
        var phaseCells=PHASES.map(function(ph){
          ...
          return '<td style="border-left:2px solid '+ph.color+'30;font-size:10px;font-weight:600;color:'+ph.color+'">'+lod+'</td>'+
        ...
        return '<tr>'+
          ...
          (d.file_format||'--')+'<br><span style="color:var(--text3)">'+(d.sheet_size||'')+(d.scale?' '+d.scale:'')+'</span></td>'+
          '<td style="font-size:10px;max-width:110px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+
          (d.bim_model_ref?'<span style="background:var(--brand-light);color:var(--brand);padding:1px 5px;border-radius:3px;border:1px solid var(--brand-border);font-size:9px">'+d.bim_model_ref+'</span>':'<span style="color:var(--text3)">--</span>')+
          '</td>'+
          phaseCells+
          (canEdit||canDel?'<td><div style="display:flex;gap:3px">'+
    ...
    document.getElementById('del-table').innerHTML='<div class="card"><div class="empty"><div class="empty-title">Error al cargar</div><div class="empty-desc">'+e.message+'</div></div></div>';
  });
}
...
function openDeliverableModal(id){
  ...
  getD.then(function(d){
    ...
    var fieldInputs=APP.schemas.sort(function(a,b){return a.code_order-b.code_order;}).map(function(s){
      ...
      return '<div class="form-group"><label class="label">'+s.name+(s.is_required?' *':'')+
        (s.is_part_of_code?'<span class="code-seg" style="margin-left:4px;font-size:8px">cod.#'+s.code_order+'</span>':'')+
        '</label><input type="text" class="input schema-field" data-key="'+s.key+'" maxlength="'+s.max_length+'" value="'+val+'" oninput="updateCodePreview()"></div>';
    }).join('');
    ...
    var phaseBlocks=PHASES.map(function(ph){
      return '<div style="border:1px solid '+ph.color+'40;border-radius:var(--r);padding:14px;background:'+ph.color+'08;margin-bottom:10px">'+
        '<div style="font-size:11px;font-weight:700;color:'+ph.color+';text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px">'+
        ph.label+(ph.sub?' - '+ph.sub:'')+'</div>'+
        '<div class="form-grid">'+
    ...
    document.getElementById('modal-container').innerHTML=
      ...
      '<div class="form-grid" style="margin-bottom:16px">'+fieldInputs+'</div>'+
      ...
      '<input type="text" class="input" id="del-predecessors" value="'+(d?d.predecessors||'':'')+'" placeholder="Ej: codigo del entregable predecesor"></div>'+
      '</div>'+
      ...
      '<input type="text" class="input" id="del-bimref" value="'+(d?d.bim_model_ref||'':'')+'" placeholder="Ej: codigo del modelo o ruta en ACC/Revizto"></div>'+

      '<div style="font-size:10px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:10px">4. Informacion por fase RIBA</div>'+
      phaseBlocks+
...
function confirmDelete(id,code){
  document.getElementById('modal-container').innerHTML=
    ...
    '<div style="background:var(--bg);padding:10px;border-radius:var(--r);border:1px solid var(--border)">'+
...
function deleteDeliverable(id){
  sbPatch('deliverables','id=eq.'+id,{is_active:false})
    .then(function(){closeModal('confirm-modal');toast('Entregable eliminado.');loadDeliverables();})
...
function renderProgress(){
  ...
  document.getElementById('content').innerHTML=
    ...
    '<p class="page-sub">Unidades productivas y avance por fase RIBA</p></div></div>'+loading();
    ...
    var html=
      ...
      phaseStats.map(function(ps){
        ...
        return '<div class="card" style="padding:14px;border-top:3px solid '+ps.ph.color+'">'+
          '<div style="font-size:10px;font-weight:700;color:'+ps.ph.color+';text-transform:uppercase;margin-bottom:10px">'+ps.ph.label+'</div>'+
          '<div style="font-size:22px;font-weight:700;font-family:\'Space Grotesk\',sans-serif;color:var(--text)">'+pct+'%</div>'+
      ...
      Object.entries(byDisc).map(function(e){
        var disc=e[0];var data=e[1];
        var pct=data.plan>0?Math.round(data.cons/data.plan*100):0;
        var pctDel=data.total>0?Math.round(data.comp/data.total*100):0;
        return '<div class="prog-row">'+
          '<div class="prog-label" style="font-size:10px;font-weight:600">'+disc+'</div>'+
          '<div class="prog-bar-wrap">'+
          '<div class="prog-track"><div class="prog-fill" style="width:'+pct+'%;background:'+progColor(pct)+'"></div></div>'+
          '<div style="font-size:9px;color:var(--text3);margin-top:2px">'+data.comp+'/'+data.total+' ent. · unid: '+data.cons+'/'+data.plan+'</div>'+
          '</div><div class="prog-pct">'+pct+'%</div></div>';
      }).join('')||'<p style="color:var(--text3);font-size:12px">Sin datos de produccion.</p>'+
      '</div>'+
      '<div class="card" style="padding:18px">'+
      '<div style="font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:14px">Distribucion por estado</div>'+
      Object.entries(STATUS_CFG).map(function(e){
        ...
        return '<div class="stat-bar-row">'+
          '<div class="stat-bar-label">'+e[1].label+'</div>'+
      ...
      '<th>Entregable</th><th>Disc.</th>'+
      '<th>Unid. Plan.</th><th>Unid. Cons.</th><th>Avance Prod.</th>'+
      PHASES.map(function(ph){return '<th style="color:'+ph.color+';font-size:9px">'+ph.label+' Fecha</th>';}).join('')+
      '<th>Estado</th>'+
      ...
      deliverables.map(function(d){
        ...
        var today=new Date();
        var phaseDateCells=PHASES.map(function(ph){
          var dt=d[ph.key+'_delivery_date'];
          if(!dt)return '<td style="font-size:10px;color:var(--text3)">--</td>';
          var isOverdue=new Date(dt)<today&&d.status!=='approved'&&d.status!=='issued';
          return '<td style="font-size:10px;'+(isOverdue?'color:var(--red);font-weight:600':'color:var(--text2)')+'">'+
...
function openProgressModal(delId,code,plan,cons){
  ...
  document.getElementById('modal-container').innerHTML=
    ...
    '<div class="modal-body">'+
    '<div style="background:var(--bg);border-radius:var(--r);padding:10px;margin-bottom:14px;border:1px solid var(--border)">'+
    '<span class="code-chip" style="font-size:9px">'+code+'</span></div>'+
    '<div class="form-grid">'+
    '<div class="form-group"><label class="label">Unidades planificadas</label>'+
    '<input type="number" class="input" id="prog-plan" value="'+plan+'" min="0"></div>'+
    '<div class="form-group"><label class="label">Unidades consumidas</label>'+
    '<input type="number" class="input" id="prog-cons" value="'+cons+'" min="0"></div>'+
    '</div></div>'+
    '<div class="modal-footer"><button class="btn" onclick="closeModal(\'prog-modal\')">Cancelar</button>'+
    '<button class="btn btn-primary" onclick="saveProgress(\''+delId+'\')">Guardar en Supabase</button></div></div></div>';
}
...
function renderSchemas(){
  ...
  sbGet('field_schemas','?project_id=eq.'+APP.project.id+'&is_active=eq.true&order=code_order.asc').then(function(schemas){
    ...
    document.getElementById('content').innerHTML=
      '<div class="page-header"><div><h1 class="page-title">Configuracion de campos</h1>'+
      '<p class="page-sub">Estructura de codificacion UKHA - '+schemas.length+' campos activos</p></div></div>'+
      ...
      '<div>'+schemas.map(function(s){
        return '<div class="schema-item">'+
          ...
          '<button class="btn btn-ghost btn-sm" onclick="openSchemaModal(\''+s.id+'\')">'+
          ...
          '<button class="btn btn-ghost btn-sm" style="color:var(--red)" onclick="confirmDeleteSchema(\''+s.id+'\',\''+s.name+'\')">'+
...
function openSchemaModal(id){
  ...
  document.getElementById('modal-container').innerHTML=
    ...
    '<div class="modal-header"><div class="modal-title">'+(s?'Editar: '+s.name:'Nuevo campo')+'</div>'+
    ...
    '<input type="text" class="input" id="sch-name" value="'+(s?s.name:'')+'" placeholder="Ej: Disciplina"></div>'+
    '<div class="form-group"><label class="label">Tipo *</label>'+
    '<select class="input" id="sch-type" onchange="document.getElementById(\'sch-vals-wrap\').style.display=this.value===\'dropdown\'?\'\':\' none\'">'+
    ...
    '<input type="number" class="input" id="sch-maxlen" value="'+(s?s.max_length:10)+'" min="1" max="50"></div>'+
    '<div class="form-group"><label class="label">Posicion en codigo</label>'+
    '<input type="number" class="input" id="sch-order" value="'+(s?s.code_order:APP.schemas.length+1)+'" min="1" max="20"></div>'+
    ...
    '<label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:12px">'+
    '<input type="checkbox" id="sch-code"'+(!s||s.is_part_of_code?' checked':'')+'>En codigo</label></div>'+
    ...
    '</div></div>'+
    '<div class="modal-footer">'+
    '<button class="btn" onclick="closeModal(\'sch-modal\')">Cancelar</button>'+
...
function saveSchema(id){
  ...
  var payload={name:name,key:key,field_type:type,
    is_required:document.getElementById('sch-req').checked,
    is_part_of_code:document.getElementById('sch-code').checked,
    code_order:parseInt(document.getElementById('sch-order').value)||1,
    separator:document.getElementById('sch-sep').value,
    max_length:parseInt(document.getElementById('sch-maxlen').value)||10,
    allowed_values:vals,project_id:APP.project.id};
...
function renderUsers(){
  ...
  sbGet('users','?select=*&order=full_name.asc').then(function(users){
    ...
    document.getElementById('content').innerHTML=
      ...
      '<p class="page-sub">'+users.length+' usuarios registrados en Supabase</p></div>'+
      ...
      users.map(function(u){
        ...
        return '<div class="perm-card">'+
          ...
          '<div class="perm-name">'+u.full_name+'</div>'+
...
function openUserModal(id){
  ...
  document.getElementById('modal-container').innerHTML=
    ...
    '</div></div>'+
    '<div class="modal-footer">'+
    '<button class="btn" onclick="closeModal(\'user-modal\')">Cancelar</button>'+
...
function saveUser(id){
  ...
  btn.disabled=true;btn.textContent='Guardando...';
  var payload={
    full_name:name,role:document.getElementById('u-role').value,
  ...
  if(id){
    ...
    if(!email||!pass){toast('Correo y contrasena obligatorios.','error');btn.disabled=false;btn.textContent='Crear usuario';return;}
  ...
  p.then(function(){
    ...
    toast(e.message,'error');btn.disabled=false;btn.textContent=id?'Actualizar':'Crear usuario';
  });
}
